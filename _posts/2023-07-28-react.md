...
1.Unlike browser DOM elements, React elements are plain objects, and are cheap to create.**本质就是object类型的一般对象**

2.虚拟DOM比较轻，携带的属性比较少，真实DOM比较重。因为虚拟DOM是react内部在用，无需真实DOM那么多属性，就可以满足需求。

3.虚拟DOM节点最终会被React转化为真实DOM节点，呈现在页面上。

4.React Element 是虚拟 DOM 的构成部分。当你使用 JSX 或者 React.createElement() 方法创建 React Element 时，实际上是在创建虚拟 DOM 中的一个**节点**。虚拟 DOM 是以 JavaScript 对象的形式表示**整个 DOM  树**，其中包含了由 React Element 构成的组件树结构。

## JSX基础语法

全称JavaScript XML，react定义的一种类似于XML的JS扩展语法：JS+XML。使用JS和JSX都可以创建虚拟DOM节点，但是JS创建虚拟DOM比较繁琐，尤其是标签如果很多的情况下，所以还是比较推荐使用JSX来创建。**使用JSX 语法来创建虚拟DOM节点，也就是React Element**，然后使用babel来编译，babel其实最终编译成还是JS来创建的虚拟DOM的语法，所以JSX其实就是JS创建虚拟DOM的一个语法糖。

所谓语法糖（Syntactic sugar）是编程语言中一种特性，它并没有引入新的功能，而是提供了一种更简洁、更易读的语法形式，使得代码更加易于理解和编写。

jsx语法规则：

1.定义虚拟DOM节点，不能使用引号“”

2.标签中混入**JS表达式**的时候使用{}, 并不可以混入Js语句

3.样式的类名指定不要使用class，使用className

4.内联样式要使用双大括号包裹，要用style = {{key:value}}的形式去写

5.不能有多个根标签，只能有一个根标签，将所有的标签放进一个div当中。

6.标签必须闭合

7.如果标签是小写字母开头，比如<good>，就将标签转化为html同名元素，如果html中无该标签对应的元素，console就报错；

​    如果是大写字母开头，react就去渲染对应的组件，如果没有定义这个组件，就会报错

8.在虚拟dom中，react会自动遍历数组，但是不会遍历对象。

> 关于JS表达式和JS语句：
>
>   JS表达式：会返回一个值，可以放在任何一个需要值的地方。例如： a，  a+b ， demo(a)，  arr.map() ，function text(){}函数本身也算一个值，一个表达式
>   JS语句：if(){ }； for(){ } ；while(){ } ；swith(){ }  等不会返回一个值，只是控制代码的逻辑走向，不属于表达式。
...
理解：模块只拆分js，div盒子拆分html，那么组件会将所有实现header功能代码和资源的结合。都整个打包带走。所以组件是用来实现**局部功能效果的代码和资源的集合**(html/css/js/image等等)，作用就是复用编码，简化项目编码，提高运行效率。**Components are like functions that return HTML elements or we can say return virtual DOM( react element ).**

> **注意：** 组件名称必须以大写字母开头。小写字母开头的组件视为原生 DOM 标签。例如，< div />` 代表 HTML 的 div 标签，而 `< Weclome /> 则代表一个组件，并且需在作用域内使用 `Welcome`，传递的参数，不能在组件中改动
...
React 解析组件标签，找到了Welcome组件。发现组件是使用函数定义的，随后调用该函数，将返回的虚拟dom转为真实dom，随后呈现在页面中。
