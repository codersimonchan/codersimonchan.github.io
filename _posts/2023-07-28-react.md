...
理解：模块只拆分js，div盒子拆分html，那么组件会将所有实现header功能代码和资源的结合。都整个打包带走。所以组件是用来实现**局部功能效果的代码和资源的集合**(html/css/js/image等等)，作用就是复用编码，简化项目编码，提高运行效率。**Components are like functions that return HTML elements.**
