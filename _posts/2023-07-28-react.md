...
全称JavaScript XML，react定义的一种类似于XML的JS扩展语法：JS+XML。使用JS和JSX都可以创建虚拟DOM，但是JS创建虚拟DOM比较繁琐，尤其是标签如果很多的情况下，所以还是比较推荐使用JSX来创建。**使用JSX 语法来创建虚拟DOM**，然后使用babel来编译，babel其实最终编译成还是JS来创建的虚拟DOM的语法，所以JSX其实就是JS创建虚拟DOM的一个语法糖。
...
理解：模块只拆分js，div盒子拆分html，那么组件会将所有实现header功能代码和资源的结合。都整个打包带走。所以组件是用来实现**局部功能效果的代码和资源的集合**(html/css/js/image等等)，作用就是复用编码，简化项目编码，提高运行效率。
